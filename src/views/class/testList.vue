<template>
  <div class="app-container test_list">
    ...
    <div class="big-box1200">
      ...
      <div class="list-gbtn">
        ...
        <div v-if="user.roles.toString()!=['student']">
          <el-button type="primary" icon="el-icon-plus" size="small" @click="addtest">添加测试</el-button>
        </div>
      </div>
      <div class="block">
        <el-timeline>
            <el-timeline-item v-for="(item, index) in GetAllTestByTeacherId" :key="index" timestamp="2019/3/12 12:12:12" placement="top">
                <el-card class="test-card" shadow="hover" :style="{backgroundColor: '#ecf5ff'}">
                  <router-link class="test_box" :to="'/class/test_detail/'+item.Id">
                    <div>
                    ...
                    <div>
                      <p class="test_state"><span class="test_time">2019/03/27 19:56:33</span><span class="test_time">2019/04/07 19:56:33</span>
                      <el-tag size="small" v-if="item.Status==0" type="success"><span>在测</span></el-tag>
                      <el-tag size="small" v-else type="warning"><span>未在测</span> </el-tag></p>
                    </div>
                  </router-link>
                  <el-button v-if="user.roles.toString()!=['student']" type="primary" icon="el-icon-edit" size="mini" @click="toEdit(index)"></el-button>
    ...
    <el-dialog title="测试编辑" :visible.sync="dialogFormVisible" >
      <el-form :model="toupdate" :rules="editFormRules">
        <el-form-item label="测试名称" prop="Title">
          <el-input v-model="toupdate.Title" placeholder="输入测试名称" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="测试时长" prop="LimiteTime">
            <el-input v-model="toupdate.LimiteTime" placeholder="输入该测试多少分钟内完成" autocomplete="off"></el-input>
        </el-form-item>
          <el-form-item label="测试状态" prop="Status">
          <el-select v-model="toupdate.Status" placeholder="请选择测试状态">
